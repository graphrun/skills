...
`contract.coverage_unbound_graph_boundary` is an accurate non-blocking warning when an internal graph boundary intentionally has no separate contract. Classify its target against the current graph before deciding completion: synchronous requests are API/HTTP, data-access interactions are data, and asynchronous-message or event-publish edges are message scope. Keep every warning in the requested protocol scope pending until it is resolved or explicitly accepted as out of scope, and report warnings in other protocols without expanding the task.
