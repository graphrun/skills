...
For an atomic Database batch, prove both commit and a failure after an earlier row has staged a write. With the `orders` configuration in [graph-authoring.md](graph-authoring.md), start from `{ "orders": {} }` and payload `{ "newOrder": { "id": "ord-1", "status": "new" }, "confirmedOrder": { "id": "ord-1", "status": "confirmed" } }`. The successful insert-then-update leaves `ord-1` confirmed and preserves the incoming payload. For rollback, change `confirmedOrder.id` to `ord-missing`: the second row fails with `database.record_missing`, the original empty collection remains unchanged, and the caller's catch determines the final outcome.

Inspect `DB_TRANSACTION_COMMITTED` or `DB_TRANSACTION_ROLLED_BACK` and state evidence, using the current MCP projection rather than assuming raw event argument fields. A successful batch publishes one state update. A rollback identifies the failed row and emits no successful mutation evidence for discarded writes. The **Atomic database batch** starter also provides successful and rolled-back two-write journeys.

For Gateway balancing, run four arrivals through three paired backends and verify A/B/C/A in selected connections and caller responses. A new run resets selection to A. Retries must remain on their originally selected backend. Potential-route coverage may include all backends; it is not evidence that all were invoked. The **Round-robin gateway** starter provides this routing journey.
